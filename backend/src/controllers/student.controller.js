...
async function getStudentCourses(req, res, next) {
  try {
    ...
    // Fetch student profile to get student_id if needed, though course service should use userId/studentId
    const student = await studentQueries.getStudentByUserId(userId);
    if (!student) {
        return sendSuccess(res, [], 200); // Return empty list if no student profile
    }

    const realStudentId = student.student_id || student.user_id;

    // Call the service function to get the list of courses
    const courses = await courseService.getCoursesByStudent(realStudentId);
